...
- **Failed/deleted/private world, garbage response, or non-2xx/3xx status (except 401/403):** Negative-cached for 60 s; both `getFriends()` reconcile and live `enrichPipelineEvent` suppress re-fetches inside that window. A transient API blip self-heals on the next reconcile/event after the window expires.
