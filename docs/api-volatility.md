...
| `GET /api/1/image/{fileId}/{version}/{size}` | Avatar thumbnails (VRX-202) | 🟡 Requires the auth cookie (401 unauthenticated); 302s to a signed CDN URL on hosts VRChat rotates freely | Redirect followed only from allowlisted issuers (≤2 hops); any failure → letter placeholder + 30s negative cache, never a crash |
