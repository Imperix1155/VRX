...
| `POST /auth/twofactorauth/{totp\|emailotp}/verify`                                   | 2FA code verification — **route by code type, never interchangeable** (`otp/verify` = recovery codes ONLY; email codes there are always rejected — VRX-229; full table in the 2FA section below)                                                            | ✅ totp live-tested (VRX-42); 🟡 emailotp triple-confirmed 2026-07-28 (spec + live probe + VRCX) — flips ✅ on the external tester's first successful login                                                                                                                                                                                                                                                                | Unexpected `verified` value → login fails (safe default)                                                                                                                                                                   |
...
**Verification:** ✅ totp: VRX-42 live testing. 🟡 emailotp: contract triple-confirmed 2026-07-28 (API reference + live 401-not-404 probe + VRCX source); end-to-end confirmation = the external tester's first successful login on 0.11.1.
