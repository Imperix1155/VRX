...
Clicking a friend row (or Enter/Space on the focused row) opens the **friend-details drawer** — `FriendDrawer.tsx`. The row's click target is a stretched native **details-opener button** (the `<li>` stays purely structural; the Join pill remains an independent sibling control stacked above it), whose accessible name COMPOSES from the visible name + status + world + platform (the tab label) — never an overriding `aria-label`, so screen readers keep every §9.1 signal. Owner-decided pixels; phase 1 ships ONLY sections with real data today (Copy link / self-invite / favorite / pin / notes / history are separate issues — NO placeholder buttons).

**Shell:** fixed right-side floating card — inset **14px** top/right/bottom, width **372px**, radius **20px** (panel scale — the §3 stack model; `.glass` supplies it), real `.glass` material (NO solid underlay — that was a mock-pane workaround, never ship it); scrim `--scrim` (`rgba(0,0,0,.44)`, one value both themes); open/close = `translateX` over **260ms `cubic-bezier(.32,.72,.29,1)`**, `motion-safe:` prefixed. The panel stays mounted while closed (translated off-screen, `inert` + `aria-hidden`) so the exit transition can play. Drawer CONTENT may scroll (it's content, not a control surface — the §8 no-scroll rule is silent here).

**A11y (hard):** `role="dialog"` + `aria-modal`, `aria-label` = the friend's name; focus trapped while open; Esc, scrim/outside click, and the ✕ button (28px, radius 9px, ghost-button styling, top-right) all close — every close path through ONE handler; Tab reaches every action; focus RETURNS to the opening row on close (falling back to the friends search input when the row no longer exists — focus never drops to `<body>`).
...
3. **WHERE** — uppercase 10.5px letterspaced label; world name 14px/600 truncating + the SHARED `InstancePill` (identical tier logic/labels as the row); Ask Me/DND show **"Hidden"** as the world; hidden-but-in-world shows "Private" (same rule as the row's pill); trust line `Trust: {rank}` (VRChat only, when known).
