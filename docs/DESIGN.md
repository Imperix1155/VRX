...
copy. After this failure on completed VRChat 2FA, both login surfaces clear the
submitted code and return to the credentials form while retaining that message;
every other 2FA failure stays on the retryable code prompt, and every other
login failure keeps the generic message.
