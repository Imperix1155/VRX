...
body::before {
  ...
  background:
    ...
    radial-gradient(58% 50% at 92% 96%, rgba(243, 113, 30, 0.2), transparent 60%), var(--bg-base);
}
body::after {
  position: fixed;
  inset: 0;
...
- **Avatar far-left** (real image, main-fetched as a CSP-safe `data:` URL — VRX-48; the letter placeholder holds during load/failure), **wrapped in a status-color ring with a status glyph badge.** Ring + glyph + the avatar's `aria-label` carry the privacy tier — VRChat's set status, or for a statusless in-game friend the tier-2 **Online** fold (VRX-207: the state palette never leaks into the ring as a platform difference) — color **+** glyph **+** text, never color-alone. Glyphs: Online ✓ (incl. statusless in-game) · Join Me ⇥ · Ask Me ? · DND – · web-active dot · offline none. The gamepad glyph is RETIRED from the row. The glyph is knocked out to `--bg-base` so it flips in light mode.
...
- **Right side: ONE instance-type pill** (text-only openness label, neutral, `min-width` + centered → a tidy right-aligned column) that **doubles as the join target** — merges the old subline openness badge **and** the separate Join/Ask/⊘ affordance into one element ("press the instance type to join"). Shows the accurate openness label — **tier-colored per §6.1** — when the instance is visible, or a neutral **"Private"** for ANY friend in a hidden world (revised 2026-07-01: any status, not just Ask Me/DND — `state === 'in-game'` is the gate); nothing only when truly not in a world (offline / web-active). **VRX-166 built:** a joinable pill is a real ordinary-Tab-stop `button` with the same rest recipe, house focus ring, and control-token hover/pressed states; non-joinable pills remain `span`s. On denial its visible label briefly reads the localized failure copy in a polite live region, then restores. The affordance passes R12 in black and white because button role, pointer cursor, and focus ring carry interactivity without color.
