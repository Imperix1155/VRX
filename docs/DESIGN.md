...
- Disconnect and logout do not silently relink, unlink, or expose another account scope. The healthy side remains usable, and unavailable identities are labeled without treating missing data as offline. Replace is disabled when the account to keep is absent from the roster; replacing the missing member through its healthy counterpart remains available. Empty account names use the same localized fallback for the drawer heading and accessible name.
