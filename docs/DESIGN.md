...
Clicking a friend row (or Enter/Space on the focused row) opens the **friend-details drawer** — `FriendDrawer.tsx`. Owner-decided pixels; phase 1 ships ONLY sections with real data today (Copy link / self-invite / favorite / pin / notes / history are separate issues — NO placeholder buttons).
...
**A11y (hard):** `role="dialog"` + `aria-modal`, `aria-label` = the friend's name; focus trapped while open; Esc, scrim/outside click, and the ✕ button (28px, radius 9px, ghost-button styling, top-right) all close; Tab reaches every action; focus RETURNS to the opening row on close.
