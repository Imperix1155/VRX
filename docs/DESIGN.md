...
- **Right side: ONE instance-type pill** (text-only openness label, neutral, `min-width` + centered → a tidy right-aligned column) that **doubles as the join target** — merges the old subline openness badge **and** the separate Join/Ask/⊘ affordance into one element ("press the instance type to join"). Shows the accurate openness label when the instance is visible, or **"Private"** when an online friend's instance is hidden (Ask Me/DND); nothing when there's no instance (offline / in-menu). It is the *visual* affordance now; the click→join IPC lands with VRX-166 (then it becomes a real `button`).
