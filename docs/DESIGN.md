...
Clicking a friend row (or Enter/Space on the focused row) opens the **friend-details drawer** — `FriendDrawer.tsx`. The row's click target is a stretched native **details-opener button** (the `<li>` stays purely structural; the Join pill remains an independent sibling control stacked above it), whose accessible name COMPOSES from the visible name + status + world — never an overriding `aria-label`, so screen readers keep every §9.1 signal. Owner-decided pixels; phase 1 ships ONLY sections with real data today (Copy link / self-invite / favorite / pin / notes / history are separate issues — NO placeholder buttons).
