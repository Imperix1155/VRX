...
5. **Notes** — private 500-character editor with a quiet live counter. Normal saving and success stay silent. A rejected save shows an adjacent tokenized error alert with a `!` glyph, localized copy that says the newest draft remains local and will not survive closing VRX, and one explicit **Retry** control. Rejection stops a queued auto-save and ordinary later blurs; only Retry may save again, using the latest draft. Retry serializes through the existing save path; late completions cannot remove a newer draft or its failure alert. Until the guarded note bridge supplies a current save revision, the editor is visibly read-only and must not accept a draft that has no save path. A terminal initial-load failure shows adjacent localized error copy and an explicit Retry that reloads the saved note before editing is enabled.

State: the selected friend id lives in the friends store (`selectedFriendId`, view state — composite `platform:platformUserId`). Future phases (VRX-70/75) add favorite/history INTO this drawer.
