...
| Function | Purpose |
| --- | --- |
| `AvatarCache` / `avatarCache.get(url)` / `setVrcCookieProvider(fn)` (`services/avatarCache.ts`) | VRX-48/202: session-memory avatar image cache (LRU 200, in-flight dedupe, 30s negative cache) behind a hard host allowlist (VRC files/api/assets + CVR `files.abinteractive.net`/`files.abidata.io`; https-only, no port/credentials). Follows the VRChat image endpoint's 302 (≤2 hops; only allowlisted hosts may ISSUE a followed redirect; targets bare-https). The VRChat auth cookie rides ONLY to `api.vrchat.cloud` via the late-wired provider (`VrcApiClient.getAuthCookieHeader`, index.ts) and is never forwarded. Serves `get-avatar` as data: URLs; separate from adapters so images never consume API rate budget |
