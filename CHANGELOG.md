...
- Private-note saves no longer fail silently. If a save is rejected, the friend drawer keeps the newest draft visible, explains that it remains only on this device and will be lost if VRX closes, and offers an explicit Retry. Until the note bridge supplies a current save revision, the editor stays read-only instead of accepting text it cannot save. (VRX-269)
