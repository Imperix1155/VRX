...
- The friends list now renders only the visible row window, stays responsive with large rosters, and supports Up/Down arrow navigation through one roving friend-details opener. Section headers remain sticky, virtual rows announce their logical list position, only visible Join buttons enter the Tab order, and the list keeps its scroll position when live friend data refreshes. (VRX-63)
