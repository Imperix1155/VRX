...
- `instance.ts` / `instance.test.ts` — `join-instance` (now friendId + renderer `expectedTarget: { worldId, instanceId }`) and VRChat-only `self-invite` (VRX-166/239/241). Both sender-guard and shape-validate before resolving through LocationAuthority and the shared joinability predicate. `join-instance` compares the authority-resolved hot instance against `expectedTarget` and returns `target-changed` WITHOUT building a URL when they differ; on match it builds the URL from main-owned data, final-validates via `isAllowedLaunchUrl`, and launches. Both actions use independent per-platform+action in-flight locks and 3s success cooldowns. Launch/invite rejections return typed `launch-failed`/`invite-failed` results; logs contain platform + reason only, never location or errors.
