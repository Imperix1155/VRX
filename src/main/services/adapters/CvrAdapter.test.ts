...
/** Tests outside restore validation bypass the auth leg deliberately; mark
 *  their synthetic sessions as owner-bound and durable for the behavior under test. */
function markSessionEstablishedForTest(adapter: CvrAdapter): CvrAdapter {
...
describe('CvrAdapter', () => {
  ...
  describe('session restore + validation (VRX-174)', () => {
    ...
    it('quarantines a restored session from the friends REST consumer', async () => {
      const fetchMock = vi.fn()
      vi.stubGlobal('fetch', fetchMock)
      const adapter = new CvrAdapter(
    ...
    it('quarantines a restored session from the instance REST consumer', async () => {
      const fetchMock = vi.fn()
      vi.stubGlobal('fetch', fetchMock)
      const adapter = new CvrAdapter(
        fakeStore({ username: 'trinity', accessKey: 'key-1' }),
  ...
  describe('instance enrichment (VRX-59)', () => {
    ...
    it('dispatches getInstanceDetails interactively ahead of pipeline-driven default resolution', async () => {
        ...
        vi.fn((url: RequestInfo | URL) => {
          ...
          return Promise.resolve(
            jsonResponse(
              envelope({
                ...instanceDetail,
                id: path.endsWith('/i_dialog') ? 'i_dialog' : 'i_pipeline'
              })
            )
          )
        })
      ...
      await vi.advanceTimersByTimeAsync(1_000)
      await expect(Promise.all([blocker, dialog])).resolves.toHaveLength(2)
      expect(dispatches).toEqual([
        { path: '/1/friends', at: 10_000 },
        { path: '/1/instances/i_dialog', at: 11_000 },
        { path: '/1/instances/i_pipeline', at: 12_000 }
      ])
    })
