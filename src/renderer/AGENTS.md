...
- `src/hooks/useExploreCoordinator.ts` is the sole renderer trigger for Explore
  ...
  unmount alone sends `setExploreActive([])`.
...
- `src/components/SettingsView.tsx`, `src/hooks/useJoinInstance.ts` — Settings → Behavior includes the neutral `allowJoinInstances` switch (VRX-39, default ON); it persists the user's choice while main remains the enforcement boundary. A main denial uses typed `joining-disabled` with specific en/ja copy.
...
- `src/stores/` — Zustand stores, one per domain; each independently testable, and **no store imports another** (compose at the view layer). Guard `window.vrx` (undefined in Preview/test) in any IPC-backed fetch. (VRX-19/21)
  ...
  - `ui.ts` — ephemeral view state ONLY: `activeTab` drives the **§8 shell nav / active view** (Dashboard/Activity/Friends/Explore/Groups/Settings), plus `drawerOpen` and the session-only `settingsCategory` (`appearance` / `behavior` / `notifications` / `accounts`). Persisted prefs like Explore count and filter live in `settings.ts`, never here.
