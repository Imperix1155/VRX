...
- `src/components/ErrorBoundary.tsx` — top-level React error boundary (VRX-127). Wraps `<App/>` in `main.tsx` (inside `QueryClientProvider`). Catches render-phase errors from its subtree and shows a glass-styled fallback (brand mark + `--error` token + non-color glyph + Reload button, all copy i18n'd). `componentDidCatch` logs to `electron-log/renderer`, which forwards to the main process via the IPC bridge wired by `initLogger()` — no new IPC channels needed. Tested via `@testing-library/react` + `// @vitest-environment jsdom` (the jsdom environment is required because React error boundaries only fire during the client commit phase). `@testing-library/react` was added to devDependencies for this purpose; `.tsx` component tests that need a real DOM use the `// @vitest-environment jsdom` header to opt in.
- `src/App.tsx`, `src/components/` — UI components. `App.tsx` is the **auth gate** (VRX-158): reads `useAuthStatus` and renders `LoginScreen` unless authenticated (blank while the check is pending, so an already-signed-in session doesn't flash the form). `LoginScreen.tsx` (VRX-158) — username/password → `window.vrx.login`, then a method-aware 2FA prompt → `window.vrx.verify2fa` for the second leg (the password is dropped from state once 2FA is requested, VRX-159); glass card with VRChat blue tint, errors via the `--error` token + a non-color glyph, all copy i18n'd; bridge/login failures are surfaced (never a silent no-op). `TokenPreview.tsx` (token-preview surface), `LocaleProbe.tsx` (throwaway i18n demo — both go when the real app shell lands), `FriendsList.tsx` (thin-slice friends list — VRX-19/21).
