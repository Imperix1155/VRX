...
- `src/stores/` — Zustand stores, one per domain; each independently testable, and **no store imports another** (compose at the view layer). Guard `window.vrx` (undefined in Preview/test) in any IPC-backed fetch. (VRX-19/21)
  ...
  - `ui.ts` — ephemeral view state ONLY: `activeTab` drives the **§8 shell nav / active view** (Dashboard/Friends/Explore/Settings), plus `drawerOpen` and the session-only `settingsCategory` (`appearance` / `behavior` / `notifications` / `accounts`). Persisted prefs like Explore count and filter live in `settings.ts`, never here.
