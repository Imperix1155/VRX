...
## Ownership
- `types.ts` — the `Friend` model and all enums (platform, presence state, status, openness, trust, linking) + auth (`Credentials`/`AuthStatus`/`LoginResult` — `AuthStatus.twoFactorMethod` accompanies the `needs-2fa` state for the reprompt flow, VRX-173), `JoinMode`, and the live `AdapterEvent` union.
