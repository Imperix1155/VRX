...
- `explore.ts` defines display-only discovery worlds, rooms, count provenance,
  load states and the 2/4/6 world-total options (default 4). Partial/unknown
  counts have a null value. Action references are opaque; only main may issue
  real references in the later live integration. Synthetic references belong
  only in tests. `exploreRanking.ts` ranks bounded platform lists separately,
  ...
  These phase-A exports have no production consumer yet (VRX-270).
