...
- `ipc.ts` — the typed IPC channel contract (`IpcInvoke` request/response + `IpcEvents` push); main↔preload↔renderer derive their types from it so a bad channel/payload is a compile error (VRX-18). `identity-boundary` carries `{ platform }` from each adapter's session boundary so the renderer can remove that account-owned friends cache (VRX-24). Instance actions accept friend IDs only (never renderer-supplied locations) and return typed expected-denial reasons (VRX-166).
