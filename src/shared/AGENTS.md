...
- `settings.ts` — the user-settings Zod schema (`Settings`, `DEFAULT_SETTINGS`, `SETTINGS_VERSION`), a versioned `runMigrations` runner (registry empty until a transforming change lands), and `parseSettings` — the safe load path: migrate → strip unknown keys → fall back to defaults on missing/invalid (`.catch`), never throws (VRX-23). Additive fields need NO version bump (`.catch` defaults cover them — e.g. `labelScheme` VRX-183, `hotInstanceThreshold` VRX-78). Persistence lives in main (`services/settings.ts`).
